...
describe("clipData", () => {
  it("returns small data unchanged and clips large data with a byte count", () => {
    ...
    expect(clipped.bytes).toBe(JSON.stringify(big).length);
    expect(clipped.preview).toBe(JSON.stringify(big).slice(0, 100));
  });
});

describe("Observer", () => {
  it("creates the run dir, writes the marker first, then one JSON line per event", async () => {
    await withRunDir(async (runDir) => {
      ...
      expect(lines[0]).toBe(generatedMarker("run1"));
      ...
      const first = events[0] as FactoryEvent;
  ...
  it("keeps a caller-supplied ts and runId and clips oversized data", async () => {
    await withRunDir(async (runDir) => {
      ...
      expect((e?.data as { preview: string }).preview.length).toBe(64);
    });
  });
  ...
  it("serialises concurrent emits in order", async () => {
    await withRunDir(async (runDir) => {
      ...
      await obs.flush();
      const events = await readEvents(runDir);
      expect(events.map((e) => e.type)).toEqual(Array.from({ length: 50 }, (_, i) => `e${i}`));
    });
  });
  ...
  it("rotates at rotationBytes and caps the number of rotated files", async () => {
    await withRunDir(async (runDir) => {
      ...
      expect((await readEvents(runDir)).map((e) => e.type)).toEqual(["e5"]);
      expect((await readEvents(runDir, "events.1.jsonl")).map((e) => e.type)).toEqual(["e4"]);
      expect((await readEvents(runDir, "events.2.jsonl")).map((e) => e.type)).toEqual(["e3"]);
      for (const name of names) {
        const firstLine = (await readFile(join(runDir, name), "utf8")).split("\n")[0];
        expect(firstLine, name).toBe(generatedMarker("run1"));
      }
    });
  });
  ...
  it("does not rotate below the threshold", async () => {
    await withRunDir(async (runDir) => {
      ...
      await obs.flush();
      expect((await readdir(runDir)).sort()).toEqual(["events.jsonl"]);
      expect(await readEvents(runDir)).toHaveLength(20);
    });
  });
  ...
  it("accepts stage, worker and git categories used by the engine", async () => {
    await withRunDir(async (runDir) => {
      ...
      await obs.flush();
      expect((await readEvents(runDir)).map((e) => e.category)).toEqual(["stage", "worker", "git"]);
    });
  });
